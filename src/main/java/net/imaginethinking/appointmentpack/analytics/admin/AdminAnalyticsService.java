...
import net.imaginethinking.appointmentpack.analytics.OperationalEventRepository;
import net.imaginethinking.appointmentpack.analytics.PageViewCountProjection;
import net.imaginethinking.appointmentpack.analytics.PatientActivityCountProjection;
...
public class AdminAnalyticsService {
    ...
    public AdminAnalyticsSummaryResponse getSummary(
        ...
        return new AdminAnalyticsSummaryResponse(
                range.from(),
                range.to(),
                operationalEventRepository.countInRange(
                        range.from(),
                        range.to()
                ),
    ...
    public OperationalEventPageResponse getEvents(
        ...
        Page<OperationalEvent> eventPage = category == null
                ? operationalEventRepository.findEventsInRange(
                range.from(),
                range.to(),
                pageRequest
        )
                : operationalEventRepository.findEventsByCategoryInRange(
                category,
                range.from(),
                range.to(),
                pageRequest
        ...
        return new OperationalEventPageResponse(
                eventPage.getContent()
                        .stream()
                        .map(OperationalEventResponse::from)
                        .toList(),
                eventPage.getNumber(),
                eventPage.getSize(),
    ...
    private AdminAnalyticsSummaryResponse.UserMetrics buildUserMetrics(AnalyticsRange range) {
        return new AdminAnalyticsSummaryResponse.UserMetrics(
                ...
                operationalEventRepository.countDistinctUsersInRange(
                        range.from(),
                        range.to()
                )
        );
    ...
    private AdminAnalyticsSummaryResponse.AuthenticationMetrics buildAuthenticationMetrics(AnalyticsRange range) {
        ...
        return new AdminAnalyticsSummaryResponse.AuthenticationMetrics(
                ...
                countAuthentication(
                        AuthenticationAction.MFA_CHALLENGE,
                        AuthenticationOutcome.CREATED,
                        range
                ),
                mfaLoginSucceeded,
                countAuthentication(
                ...
                countAuthentication(
                        AuthenticationAction.MFA_SETUP,
                        AuthenticationOutcome.ENABLED,
                        range
                ),
                countAuthentication(
    ...
    private AdminAnalyticsSummaryResponse.DocumentProcessingMetrics buildDocumentProcessingMetrics(AnalyticsRange range) {
        return new AdminAnalyticsSummaryResponse.DocumentProcessingMetrics(
                ...
                countProcessingFailure(
                        DocumentProcessingFailureReason.TIMEOUT,
                        range
                ),
                countProcessingFailure(
                        DocumentProcessingFailureReason.SERVICE_UNAVAILABLE,
                        range
                ),
                countProcessingFailure(
                        DocumentProcessingFailureReason.PROCESSING_ERROR,
                        range
                ),
                countProcessingFailure(
                        DocumentProcessingFailureReason.UNKNOWN,
                        range
                )
        );
    }
    ...
    private AdminAnalyticsSummaryResponse.PatientActivityMetrics buildPatientActivityMetrics(AnalyticsRange range) {
        List<PatientActivityCountProjection> counts =
                operationalEventRepository.countPatientActivityInRange(
        ...
        long total = counts.stream()
                .mapToLong(PatientActivityCountProjection::getEventCount)
                .sum();
        ...
        return new AdminAnalyticsSummaryResponse.PatientActivityMetrics(
                total,
                breakdown
        );
    }
    ...
    private AdminAnalyticsSummaryResponse.PageViewMetrics buildPageViewMetrics(AnalyticsRange range) {
        ...
        long total = counts.stream()
                .mapToLong(PageViewCountProjection::getEventCount)
                .sum();

        List<AdminAnalyticsSummaryResponse.PageViewCount> breakdown = counts.stream()
                .map(item -> new AdminAnalyticsSummaryResponse.PageViewCount(
                        item.getPage(),
                        item.getEventCount()
                ))
                .toList();

        return new AdminAnalyticsSummaryResponse.PageViewMetrics(
                total,
                breakdown
        );
    }

    private long countAuthentication(
            AuthenticationAction action,
            AuthenticationOutcome outcome,
            AnalyticsRange range
    ) {
        return operationalEventRepository.countAuthenticationInRange(
                action,
                outcome,
                range.from(),
                range.to()
    ...
    private long countProcessing(
        ...
        return operationalEventRepository.countProcessingInRange(
                operation,
                outcome,
                range.from(),
                range.to()
    ...
    private long countProcessingFailure(
            DocumentProcessingFailureReason reason,
            AnalyticsRange range
    ) {
        return operationalEventRepository.countProcessingFailureReasonInRange(
                reason,
                range.from(),
                range.to()
        );
    ...
    private Long averageProcessingDuration(
        ...
        Double average = operationalEventRepository
                .averageProcessingDurationInRange(
                        operation,
                        outcome,
                        range.from(),
                        range.to()
    ...
    private AnalyticsRange resolveRange(
            Instant from,
            Instant to
    ) {
        Instant resolvedTo = to == null
                ? Instant.now()
                : to;

        Instant resolvedFrom = from == null
                ? resolvedTo.minus(DEFAULT_RANGE)
                : from;

        if (!resolvedFrom.isBefore(resolvedTo)) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST,
                    "Analytics 'from' must be before 'to'"
            );
        }
